...
const BonusSection = () => {
  ...
  const bonuses = [
    {
      ...
      fallbacks: [
        "/images/bonus-sobremesas.png",
        "/images/bonus-sobremesas.jpg",
      ],
      title: "BONO 1:",
      subtitle: "200 POSTRES",
      subtitle2: "SIN AZÚCAR",
      originalPrice: "$4.50 USD",
      placeholder: "🍰",
  ...
  const RobustBonusImage = ({ bonus, className = "" }) => {
    ...
    if (hasError || imageErrors[bonus.id]) {
      return (
        <div
          className={`w-full h-full bg-gradient-to-br ${bonus.bgColor} flex items-center justify-center ${className}`}
        >
      ...
      <div className="container mx-auto px-4 max-w-6xl">
        ...
        {Object.keys(imageErrors).length > 0 && (
          <div className="text-center mt-4 text-xs text-gray-500 opacity-50">
            ⚠️ {Object.keys(imageErrors).length} imágenes bonus usando
            placeholder
          </div>
        )}
      </div>
